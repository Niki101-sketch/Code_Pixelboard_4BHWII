...
EntprellterTaster::EntprellterTaster(int pin) 
    : pin(pin), 
      ...
      letzteAenderung(0), 
      gedruecktStartzeit(0), 
      wurdeBereitsGemeldet(false) {
...
bool EntprellterTaster::wurdeGedrueckt() {
    ...
    if (!entprellterZustand && !wurdeBereitsGemeldet && 
        (jetzt - gedruecktStartzeit < 1000) && gedruecktStartzeit > 0) {
        wurdeBereitsGemeldet = true;
        return true;
    }
    return false;
}
